...
alter table blog_related enable row level security;
...
create policy "public read blog_related" on blog_related for select using (true);
create policy "admin write blog_related" on blog_related for all using (is_admin()) with check (is_admin());
...
-- ═══════════════════════════════════════════════════════════════════════════
--  9. branches — 필한방병원 전용 확장 (참고구현 settings_business는 단일 지점
--     전제라 4개 지점 네트워크에 안 맞음. 지점별 NAP·병원장·JSON-LD용 데이터.)
-- ═══════════════════════════════════════════════════════════════════════════
create table if not exists branches (
  id               uuid primary key default gen_random_uuid(),
  name             text not null,              -- "대전필한방병원(본원)"
  slug             text unique not null,        -- "daejeon"
  is_main          boolean not null default false,
  director_id      uuid references staff(id) on delete set null,
  established_on   date,
  bed_info         text,                        -- "70병상" / "1·2인실 입원실"
  street_address   text not null,
  address_locality text,
  address_region   text,
  postal_code      text,
  address_country  text not null default 'KR',
  latitude         numeric,
  longitude        numeric,
  telephone        text not null,
  opening_hours    jsonb default '[]'::jsonb,   -- [{day, open, close}] 또는 schema.org 형식 문자열 배열
  price_info       text default '비급여 항목은 전화 문의',
