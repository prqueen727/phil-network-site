...
export async function getPublishedBlogs(limit?: number): Promise<MediaListItem[]> {
  ...
  let query = supabase
    .from("blogs")
    .select("id, title, slug, excerpt, published_at, featured_image:media!blogs_featured_image_id_fkey(storage_path, alt)")
    .eq("status", "published")
...
export async function getPublishedBlogsPage(page: number, pageSize: number): Promise<{ posts: MediaListItem[]; total: number }> {
  ...
  const { data, error, count } = await supabase
    .from("blogs")
    .select("id, title, slug, excerpt, published_at, featured_image:media!blogs_featured_image_id_fkey(storage_path, alt)", { count: "exact" })
    .eq("status", "published")
    .order("published_at", { ascending: false })
    .range(from, to);
...
export async function getPublishedBlogBySlug(slug: string) {
  ...
  const { data, error } = await supabase
    .from("blogs")
    .select("*, featured_image:media!blogs_featured_image_id_fkey(storage_path, alt), author:staff(name, job_title, slug, bio, specialty, alumni_of, career, photo_url)")
    .eq("status", "published")
    .eq("slug", slug)
    .maybeSingle();
