/**
 * jsonld.ts — JSON-LD(@graph) 조립 순수함수
 * ------------------------------------------------------------------
 * 04_supabase_참고구현/lib/buildJsonLd.ts를 TypeScript로 이식.
 * ★ 이 파일은 DB/스택에 전혀 의존하지 않는다. 데이터를 인자로 받아 JSON-LD만 만든다.
 *   호출자(API 라우트)가 Supabase에서 읽어온 데이터를 넘겨준다.
 *
 * @graph = [ BlogPosting, [업종]Business, FAQPage(있으면) ]
 */
...
/** faqs → FAQPage (이번 범위에는 FAQ 입력 UI가 없어 항상 빈 배열로 호출되지만, 추후 확장을 위해 시그니처는 유지) */
